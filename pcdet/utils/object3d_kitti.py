...
class Object3d(object):
    def __init__(self, line):
        ...
        self.h = float(label[6])  # 8
        self.w = float(label[5])  # 9
        self.l = float(label[4])  # 10
        # self.loc = np.array((float(label[11]), float(label[12]), float(label[13])), dtype=np.float32)
        # -y,z+2.3,x;yzx->xyz
        self.loc = np.array((float(label[1]), float(label[2]), float(label[3])), dtype=np.float32)  # 13,11,12
        #self.dis_to_cam = np.linalg.norm(self.loc)
        self.ry = float(label[7])  # 14
        self.score = float(label[8]) if label.__len__() == 16 else -1.0  #15
        self.level_str = None
