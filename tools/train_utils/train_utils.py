...
def train_one_epoch(model, optimizer, train_loader, model_func, lr_scheduler, accumulated_iter, optim_cfg,
    ...
    for cur_it in range(total_it_each_epoch):
        ...
        model.train()  # 1.将模型设置为训练模式
        optimizer.zero_grad()  # 2.梯度清零

        loss, tb_dict, disp_dict = model_func(model, batch)   # 3.模型前向传播

        forward_timer = time.time()
        cur_forward_time = forward_timer - data_timer

        loss.backward()  # 4.损失反向传播
        # 对梯度进行截断
        clip_grad_norm_(model.parameters(), optim_cfg.GRAD_NORM_CLIP)
        optimizer.step()  # 5.梯度更新既优化器降低损失

        accumulated_iter += 1  # 累计迭代次数+1
...
def train_model(model, optimizer, train_loader, model_func, lr_scheduler, optim_cfg,
    ...
    """
        ...
        Args:
            ...
            lr_scheduler: 学习率调度器
            optim_cfg: 优化器配置
            start_epoch: 起始epoch--> 80
            total_epochs: 总共epoch数量 -->80
            start_iter: 起始迭代数 -->1120
            rank:进程号
    ...
    with tqdm.trange(start_epoch, total_epochs, desc='epochs', dynamic_ncols=True, leave=(rank == 0)) as tbar:
        total_it_each_epoch = len(train_loader)  # 完成一个epoch所需的iter次数：54
